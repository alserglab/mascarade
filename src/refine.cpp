...
IntegerVector twoMoveBnB(NumericVector cxmin, NumericVector cxmax, NumericVector cymin, NumericVector cymax,
  ...
  auto cnt3 = [&](int c, int L, int exl, int& b, int& x, int& l) { b = 0; x = 0; l = 0;
    for (int M = 0; M < K; ++M) { if (M == L || M == exl) continue;
      if (bbf(c, cur[M])) ++b; if (llf(c, cur[M])) ++x; if (lbf(c, cur[M])) ++l; } };
  int passes = 0; std::vector<int> ord(K);
  for (int pass = 0; pass < maxpass; ++pass) { ++passes; bool changed = false;
    for (int i = 0; i < K; ++i) ord[i] = i;
    std::sort(ord.begin(), ord.end(), [&](int a, int b) { return len[cur[a]] > len[cur[b]]; });
    double minPart = 1e18;
    for (int M = 0; M < K; ++M) { double d = lv(byl[M][0]) - lv(cur[M]); if (d < minPart) minPart = d; }
    for (int oi = 0; oi < K; ++oi) { int L1 = ord[oi]; double curL1 = lv(cur[L1]);
      bool have = false; int bBB = 0, bXX = 0, bLB = 0; double bDL = 0; int bL2 = -1, bc1 = -1, bc2 = -1;
      auto better = [&](int db, int dx, int dl, double dlen)->bool {
        if (!have) return db < 0 || (db == 0 && (dx < 0 || (dx == 0 && (dl < 0 || (dl == 0 && dlen < -1e-9)))));
        if (db != bBB) return db < bBB; if (dx != bXX) return dx < bXX; if (dl != bLB) return dl < bLB; return dlen < bDL - 1e-9; };
      for (int c1 : byl[L1]) { double d1 = lv(c1) - curL1;
        bool boundBeaten = have ? (d1 + minPart >= bDL) : (d1 + minPart >= 0);
        if (boundBeaten) break;
        int nb = 0, blk = -1;
        for (int M = 0; M < K; ++M) { if (M == L1) continue; if (conf(c1, cur[M])) { if (++nb > 1) break; blk = M; } }
        if (nb == 0) {
          int b1, x1, l1; cnt3(c1, L1, -1, b1, x1, l1);
          int ob1, ox1, ol1; cnt3(cur[L1], L1, -1, ob1, ox1, ol1);
          int db = b1 - ob1, dx = x1 - ox1, dl = l1 - ol1;
          if (better(db, dx, dl, d1)) { have = true; bBB = db; bXX = dx; bLB = dl; bDL = d1; bL2 = -1; bc1 = c1; bc2 = -1; }
          continue; }
        if (nb > 1) continue;
        int L2 = blk; double curL2 = lv(cur[L2]);
        int ob1, ox1, ol1; cnt3(cur[L1], L1, L2, ob1, ox1, ol1);
        int ob2, ox2, ol2; cnt3(cur[L2], L2, L1, ob2, ox2, ol2);
        for (int c2 : byl[L2]) { double dd = d1 + (lv(c2) - curL2);
          bool bnd = have ? (dd >= bDL) : (dd >= 0);
          if (bnd) break;
          if (conf(c1, c2)) continue;
          int b1, x1, l1; cnt3(c1, L1, L2, b1, x1, l1);
          int b2, x2, l2; cnt3(c2, L2, L1, b2, x2, l2);
          int db = (b1 + b2) - (ob1 + ob2), dx = (x1 + x2) - (ox1 + ox2), dl = (l1 + l2) - (ol1 + ol2);
          if (better(db, dx, dl, dd)) { have = true; bBB = db; bXX = dx; bLB = dl; bDL = dd; bL2 = L2; bc1 = c1; bc2 = c2; } } }
      if (have && (bBB < 0 || bXX < 0 || bLB < 0 || bDL < -1e-9)) {
        cur[L1] = bc1; if (bL2 >= 0) cur[bL2] = bc2; changed = true; }
    }
    if (!changed) break;
  }
  IntegerVector out(K); for (int k = 0; k < K; ++k) out[k] = cur[k];
